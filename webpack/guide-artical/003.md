...
如果此刻我们使用import MyImage from './my-image.png'来引用一张图片的时候，webpack会处理这张图片，首先它会在输出文件夹生成一张图片，该图片跟my-image.png是一样的，并且变量MyImage保存了该图片在输出文件夹里面完整的路径。同样的处理会出现在使用css-loader处理css文件在样式中使用url引用一张图片和使用html-loader在html中加载一张图片的时候，前提是你使用了file-loader。
